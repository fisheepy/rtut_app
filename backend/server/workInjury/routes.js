...
function createWorkInjuryRouter({ uri, databaseName, requireTrainingSession }) {
  ...
  router.get('/reports/case-details.xlsx', async (_req, res) => {
    ...
    try {
      ...
      costs.columns = [{ header: 'Case ID', key: 'id', width: 26 }, { header: 'Employee', key: 'employee', width: 28 }, { header: 'Invoice Date', key: 'date', width: 16 }, { header: 'Description', key: 'description', width: 45 }, { header: 'Paid By', key: 'paidBy', width: 24 }, { header: 'Royal Cost Type', key: 'royalCostType', width: 22 }, { header: 'Other Cost Item', key: 'royalCostOtherItem', width: 34 }, { header: 'Amount', key: 'amount', width: 16 }, { header: 'Invoice Link', key: 'link', width: 50 }];
  ...
  router.get('/reports/case-costs.xlsx', async (req, res) => {
    ...
    const client = createClient();
    try { await client.connect(); const records = await client.db(databaseName).collection('work_injury_cases').find({ injuryDateTime: { $gte: from, $lte: `${to}T23:59:59.999` } }).sort({ injuryDateTime: 1 }).toArray(); const workbook = new ExcelJS.Workbook(); const sheet = workbook.addWorksheet('Case Cost Summary'); sheet.columns = [{ header: 'Case ID', key: 'id', width: 26 }, { header: 'Employee', key: 'employee', width: 28 }, { header: 'Injury Date / Time', key: 'date', width: 22 }, { header: 'Department', key: 'department', width: 22 }, { header: 'Location', key: 'location', width: 20 }, { header: 'Status', key: 'status', width: 14 }, { header: 'Paid by Workers Compensation', key: 'wc', width: 30 }, { header: 'Royal - Lost Time', key: 'lostTime', width: 22 }, { header: 'Royal - Medical Bill', key: 'medicalBill', width: 24 }, { header: 'Royal - Other', key: 'other', width: 20 }, { header: 'Royal - Uncategorized', key: 'uncategorized', width: 26 }, { header: 'Paid by Royal', key: 'royal', width: 20 }, { header: 'Total Case Cost', key: 'total', width: 20 }]; records.forEach(record => { const entries = Array.isArray(record.costs) ? record.costs : []; const wc = entries.filter(cost => cost.paidBy === 'Workers Compensation').reduce((sum, cost) => sum + (Number(cost.amount) || 0), 0); const lostTime = entries.filter(cost => cost.paidBy === 'Royal' && cost.royalCostType === 'Lost Time').reduce((sum, cost) => sum + (Number(cost.amount) || 0), 0); const medicalBill = entries.filter(cost => cost.paidBy === 'Royal' && cost.royalCostType === 'Medical Bill').reduce((sum, cost) => sum + (Number(cost.amount) || 0), 0); const other = entries.filter(cost => cost.paidBy === 'Royal' && cost.royalCostType === 'Other').reduce((sum, cost) => sum + (Number(cost.amount) || 0), 0); const uncategorized = entries.filter(cost => cost.paidBy === 'Royal' && !['Lost Time', 'Medical Bill', 'Other'].includes(cost.royalCostType)).reduce((sum, cost) => sum + (Number(cost.amount) || 0), 0); const royal = lostTime + medicalBill + other + uncategorized; sheet.addRow({ id: String(record._id), employee: record.employeeName, date: record.injuryDateTime, department: record.department, location: record.location, status: record.closedAt ? 'Closed' : 'Open', wc, lostTime, medicalBill, other, uncategorized, royal, total: wc + royal }); }); ['wc', 'lostTime', 'medicalBill', 'other', 'uncategorized', 'royal', 'total'].forEach(key => { sheet.getColumn(key).numFmt = '$#,##0.00'; }); return await sendWorkbook(res, workbook, `Work_Injury_Case_Costs_${from}_to_${to}.xlsx`); } catch (error) { console.error('Unable to create work injury cost report:', error); return res.status(500).json({ error: 'The work injury cost report could not be created.' }); } finally { await client.close(); }
  });
