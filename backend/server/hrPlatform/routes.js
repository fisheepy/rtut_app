...
function createHrPlatformRouter({ uri, databaseName, requireHrToolsSession }) {
  ...
  router.put('/terminations/:employeeId/cobra', async (req, res) => {
    ...
    try {
      ...
      const existing = await db.collection('employee_hr_termination').findOne({ employeeId });
      ...
      await db.collection('employee_hr_termination').updateOne({ employeeId }, { $set: values, $unset: { cobraClosedAt: '', cobraClosedBy: '' } }, { upsert: true });
      return res.json(values);
    } catch (error) {
  ...
  router.put('/terminations/:employeeId/cobra/close', async (req, res) => {
    const employeeId = req.params.employeeId;
    if (!ObjectId.isValid(employeeId)) return res.status(400).json({ error: 'Invalid employee.' });
    const client = createClient();
    try {
      ...
      const collection = client.db(databaseName).collection('employee_hr_termination');
      const existing = await collection.findOne({ employeeId });
      if (!existing?.cobraStartDate) return res.status(400).json({ error: 'A COBRA enrollment record is required before closing.' });
      ...
      const reviewer = clean(req.adminSession?.email).toLowerCase();
      const values = { cobraClosedAt: new Date(), cobraClosedBy: reviewer, updatedAt: new Date(), updatedBy: reviewer };
      await collection.updateOne({ employeeId }, { $set: values });
      return res.json(values);
    } catch (error) {
      console.error('Unable to close COBRA enrollment:', error);
      return res.status(500).json({ error: 'COBRA enrollment could not be closed.' });
    } finally { await client.close(); }
  });
  ...
  router.get('/terminations/reports/cobra.xlsx', async (_req, res) => {
    const client = createClient();
    try {
      await client.connect();
      ...
      sheet.columns = [
        { header: 'Employee', key: 'name', width: 28 }, { header: 'Email', key: 'email', width: 32 },
        { header: 'Termination Date', key: 'terminationDate', width: 18 }, { header: 'COBRA Start Date', key: 'startDate', width: 20 },
        { header: 'COBRA End Date', key: 'endDate', width: 20 }, { header: 'Status', key: 'status', width: 16 },
        { header: 'Last Updated By', key: 'updatedBy', width: 32 }, { header: 'Closed At', key: 'closedAt', width: 24 },
        { header: 'Closed By', key: 'closedBy', width: 32 },
      ];
      rows.sort((a, b) => clean(a.record.cobraStartDate).localeCompare(clean(b.record.cobraStartDate))).forEach(({ employee, record }) => sheet.addRow({
        name: [clean(employee['First Name']), clean(employee['Last Name'])].filter(Boolean).join(' '), email: clean(employee.Email),
        terminationDate: clean(employee['Termination Date']), startDate: clean(record.cobraStartDate), endDate: clean(record.cobraEndDate),
  ...
  router.get('/leaves', async (_req, res) => {
    ...
    try {
      ...
      const previouslyLockedTrackers = leaveRecords.filter(record => record.medicalFileTracker?.finalLockedAt || record.medicalFileTracker?.finalLockedBy);
      if (previouslyLockedTrackers.length) {
        await db.collection('employee_hr_leave').bulkWrite(previouslyLockedTrackers.map(record => {
          record.medicalFileTracker = { ...(record.medicalFileTracker || {}), finalLockedAt: null, finalLockedBy: '' };
          return { updateOne: { filter: { _id: record._id }, update: { $unset: { 'medicalFileTracker.finalLockedAt': '', 'medicalFileTracker.finalLockedBy': '' } } } };
        }));
      }
      ...
      return res.json(allLeaveRecords.map(record => {
        ...
        return {
          ...
          medicalFolderUrl: clean(record.medicalFolderUrl), payrollStartDate: clean(record.payrollStartDate), payrollEndDate: clean(record.payrollEndDate), firstPayrollAfterLeaveDate: clean(record.firstPayrollAfterLeaveDate),
          insuranceStatus: clean(record.insuranceStatus), insuranceEndDate: clean(record.insuranceEndDate), caseLogs: Array.isArray(record.caseLogs) ? record.caseLogs : [],
  ...
  router.put('/leaves/:employeeId/details', async (req, res) => {
    ...
    const values = {
      ...
      firstPayrollAfterLeaveDate: clean(req.body?.firstPayrollAfterLeaveDate), insuranceEndDate: clean(req.body?.insuranceEndDate),
    };
    if (!ObjectId.isValid(employeeId)) return res.status(400).json({ error: 'Invalid employee.' });
    if (!values.leaveStartedAt || !validDate(values.leaveStartedAt)) return res.status(400).json({ error: 'Leave Started Date is required.' });
    if ([values.anticipatedReturnDate, values.actualReturnDate, values.payrollStartDate, values.payrollEndDate, values.firstPayrollAfterLeaveDate, values.insuranceEndDate].some(value => !validDate(value))) return res.status(400).json({ error: 'Dates must use YYYY-MM-DD format.' });
    ...
    if (values.payrollStartDate && values.payrollEndDate && values.payrollEndDate < values.payrollStartDate) return res.status(400).json({ error: 'STD Approved Ending Date cannot be before the Starting Date.' });
    if (!['ongoing', 'cobra'].includes(values.insuranceStatus)) return res.status(400).json({ error: 'Select Insurance Status as Ongoing or Ended/COBRA.' });
    if (values.insuranceStatus === 'ongoing') values.insuranceEndDate = '';
    const client = createClient();
    try {
      ...
      const update = { $set: { ...values, updatedAt: now, updatedBy: clean(req.adminSession?.email).toLowerCase() }, $unset: { cobraStartDate: '' } };
      if (clean(existing.payrollStartDate) !== values.payrollStartDate || clean(existing.payrollEndDate) !== values.payrollEndDate || clean(existing.firstPayrollAfterLeaveDate) !== values.firstPayrollAfterLeaveDate) {
        Object.assign(update.$unset, { payrollCheckedAt: '', payrollCheckedBy: '', payrollFinalReviewedAt: '', payrollFinalReviewedBy: '' });
      }
      if (clean(existing.actualReturnDate) !== values.actualReturnDate || clean(existing.firstPayrollAfterLeaveDate) !== values.firstPayrollAfterLeaveDate) {
  ...
  router.put('/leaves/:employeeId/medical-file-tracker', async (req, res) => {
    ...
    try {
      ...
      if (!['save', 'check'].includes(action)) return res.status(400).json({ error: 'Invalid Medical File Check action.' });
      ...
      const saved = { ...tracker, fieldsSnapshot: catalog, updatedAt: now, updatedBy: reviewer, checkedAt: checked ? (record.medicalFileTracker?.checkedAt || now) : null, checkedBy: checked ? (clean(record.medicalFileTracker?.checkedBy) || reviewer) : '' };
  ...
  router.put('/leaves/:employeeId/payroll-review', async (req, res) => {
    ...
    try {
      ...
      if (!record) return res.status(404).json({ error: 'Open leave case not found.' });
      if (!record.payrollStartDate) return res.status(400).json({ error: 'STD Approved Starting Date is required before Payroll Admin Check.' });
      if (action === 'final-review' && !record.payrollEndDate) return res.status(400).json({ error: 'STD Approved Ending Date is required before Payroll Final Review.' });
  ...
  router.put('/leaves/:employeeId/close', async (req, res) => {
    const employeeId = req.params.employeeId; const action = clean(req.body?.action || 'request').toLowerCase();
    if (!ObjectId.isValid(employeeId) || !['request', 'approve'].includes(action)) return res.status(400).json({ error: 'Invalid Medical Leave close action.' });
    const client = createClient(); try {
      ...
      if (action === 'approve' && closedBy !== finalReviewerEmail) return res.status(403).json({ error: 'Only Myra can give Case Close Final Approval.' });
      if (!terminated && !returnedActive) return res.status(400).json({ error: 'Change Company App Status to Active or terminate the employee before requesting case closure.' });
      if (!terminated && (!record.actualReturnDate || !record.firstPayrollAfterLeaveDate)) return res.status(400).json({ error: 'Return to Work Date and First Payroll Date After Leave Ended are required before requesting case closure.' });
      if (action === 'request') {
        await collection.updateOne({ _id: record._id }, { $set: { closeRequestedAt: now, closeRequestedBy: closedBy, updatedAt: now, updatedBy: closedBy } });
        return res.json({ success: true, closeRequestedAt: now, closeRequestedBy: closedBy });
      }
      if (!record.closeRequestedAt) return res.status(400).json({ error: 'An admin close request is required before final approval.' });
      await collection.updateOne({ _id: record._id }, { $set: { active: false, caseStatus: 'Closed', closedAt: now, closedBy, closeReason: terminated ? 'Employee terminated in Company App' : 'Employee returned to work and case closure approved', employeeSnapshot: employee, updatedAt: now, updatedBy: closedBy } });
