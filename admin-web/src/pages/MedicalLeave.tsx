...
type LeaveEmployee = {
  ...
  leaveStartedAt: string | null; returnedAt: string | null; caseStatus: string;
}
...
export default function MedicalLeave() {
  ...
  return <div className="space-y-7 pb-12">
    <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-violet-950 to-fuchsia-900 px-7 py-8 text-white shadow-2xl">
      ...
      <div className="relative mt-6 flex flex-wrap items-end justify-between gap-4"><div><div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-wider"><HeartPulse className="h-4 w-4" />Protected & Medical Leave</div><h1 className="mt-3 text-4xl font-semibold">FMLA / ADA / Medical Leave</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-violet-100">Changing Company App Status to Leave opens a case here. Returning the employee to Active records the return but keeps the case visible until HR completes the remaining work.</p></div><button className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-bold text-violet-950 shadow" onClick={load}><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />Refresh from Company App</button></div>
    </section>
    ...
    <section className="grid gap-4 sm:grid-cols-3">
      <div className="rounded-2xl border border-violet-200 bg-violet-50 p-5 shadow-sm"><div className="text-xs font-bold uppercase tracking-wider text-violet-700">Open Leave Cases</div><div className="mt-2 text-3xl font-bold text-violet-950">{employees.length}</div></div>
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm"><div className="text-xs font-bold uppercase tracking-wider text-emerald-700">Company App Status</div><div className="mt-2 text-xl font-bold text-emerald-950">Active or Leave</div></div>
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="text-xs font-bold uppercase tracking-wider text-slate-500">Employment Status</div><div className="mt-2 text-xl font-bold text-slate-950">Not Terminated</div></div>
    </section>
    ...
    <section className="overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-xl"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-violet-200 bg-gradient-to-r from-violet-50 via-fuchsia-50 to-white px-6 py-5"><div><h2 className="text-xl font-bold text-violet-950">Open Leave Cases</h2><p className="mt-1 text-sm text-violet-700">Company App controls Active or Leave status. Returning to Active does not close the HR case.</p></div><span className="rounded-full bg-white px-3.5 py-1.5 text-sm font-bold text-violet-800 shadow-sm">{visible.length} employees</span></div><div className="max-h-[620px] overflow-auto"><table className="min-w-[1360px] w-full text-sm"><thead className="sticky top-0 z-20 bg-slate-100 text-left text-xs uppercase text-slate-600"><tr>{['Employee','Company App Status','HR Case','Leave Started','Returned To Active','Hire Date','Department','Job Title','Location','Supervisor','Email','Phone'].map((label, index) => <th className={`whitespace-nowrap px-4 py-3 ${index === 0 ? 'sticky left-0 z-30 bg-slate-100' : ''}`} key={label}>{label}</th>)}</tr></thead><tbody>{visible.map(employee => <tr className="border-t transition hover:bg-violet-50/40" key={employee.id}><td className="sticky left-0 z-10 min-w-56 border-r-4 border-violet-100 bg-white px-4 py-3 font-bold">{employee.name}<div className="text-xs font-normal text-slate-500">Not terminated</div></td><td className="px-4 py-3"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${employee.status.toLowerCase() === 'leave' ? 'bg-violet-100 text-violet-800' : 'bg-emerald-100 text-emerald-800'}`}>{employee.status}</span></td><td className="px-4 py-3"><span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-800">{employee.caseStatus}</span></td><td className="px-4 py-3 font-semibold">{dateDisplay(employee.leaveStartedAt)}</td><td className="px-4 py-3">{dateDisplay(employee.returnedAt)}</td><td className="px-4 py-3">{dateDisplay(employee.hireDate)}</td><td className="px-4 py-3">{employee.department || '--'}</td><td className="px-4 py-3">{employee.jobTitle || '--'}</td><td className="px-4 py-3">{employee.location || '--'}</td><td className="px-4 py-3">{employee.supervisor || '--'}</td><td className="px-4 py-3">{employee.email || '--'}</td><td className="px-4 py-3">{employee.phone || '--'}</td></tr>)}{!visible.length && <tr><td className="px-6 py-14 text-center text-slate-500" colSpan={12}>{loading ? 'Loading leave cases...' : employees.length ? 'No leave cases match these filters.' : 'No open leave cases.'}</td></tr>}</tbody></table></div></section>
  </div>
}
