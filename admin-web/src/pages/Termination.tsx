...
import { ArrowLeft, Filter, RefreshCw, Save, Search, UserMinus, X } from 'lucide-react'
...
type EditRecord = Pick<Employee, 'finalPayrollDate' | 'pendingIssues' | 'pendingIssuesNotes' | 'payrollFollowThroughUntil' | 'insuranceParticipation' | 'insuranceEndingDate' | 'retirementParticipation' | 'retirementEndingDate'>
const emptyRecord: EditRecord = { finalPayrollDate: '', pendingIssues: false, pendingIssuesNotes: '', payrollFollowThroughUntil: '', insuranceParticipation: '', insuranceEndingDate: '', retirementParticipation: '', retirementEndingDate: '' }
...
export default function Termination() {
  ...
  async function load() {
    ...
    try {
      const [records, auth] = await Promise.all([api.get('/hr-platform/terminations'), api.get('/hr-tools-auth/me')])
      setEmployees(records.data || []); setCurrentUserEmail(auth.data.email || '')
  ...
  const payroll = [...filtered].filter(employee => rangeActive || !employee.payrollFinalReviewedAt || isCurrentMonth(employee.terminationDate) || isCurrentMonth(employee.payrollFinalReviewedAt)).sort((a, b) => actionDate(a).localeCompare(actionDate(b)) || a.name.localeCompare(b.name))
  ...
  return <div className="space-y-6">
    ...
    <StatusSection title="Termination & Payroll Status" description={rangeActive ? 'Showing all employees in the selected termination date range.' : 'Unfinished items remain visible; completed terminations remain visible during the current month.'} tone="blue" count={payroll.length}>
      <table className="min-w-[1500px] text-sm"><TableHead labels={['Name','Termination Date','Department','Job Title','Location','Supervisor','Final Payroll Date','Pending Issues','Payroll Follow-through Until','Payroll Check','Payroll Final Review','Details']} /><tbody className="divide-y divide-slate-100">{payroll.map(employee => <tr className={!employee.payrollFinalReviewedAt ? 'bg-amber-50/60' : ''} key={employee.id}><StickyName employee={employee} /><Cell>{dateDisplay(employee.terminationDate)}</Cell><Cell>{employee.homeDepartment || '--'}</Cell><Cell>{employee.jobTitle || '--'}</Cell><Cell>{employee.location || '--'}</Cell><Cell>{employee.supervisor || '--'}</Cell><Cell>{dateDisplay(employee.finalPayrollDate)}</Cell><Cell>{employee.pendingIssues ? <span className="font-bold text-red-700">Pending: {employee.pendingIssuesNotes}</span> : 'No'}</Cell><Cell>{dateDisplay(employee.payrollFollowThroughUntil)}</Cell><Cell>{employee.payrollCheckedAt ? statusPill(true) : <ActionButton label="Confirm Check" onClick={() => check(employee, 'payroll-check', 'Payroll Check')} disabled={!employee.finalPayrollDate} />}</Cell><Cell>{employee.payrollFinalReviewedAt ? <div>{statusPill(true, 'Final Reviewed')}{manager && <button className="ml-2 text-xs font-bold text-red-700" onClick={() => check(employee, 'payroll-final-review-undo', 'undo Payroll Final Review')}>Correct</button>}</div> : manager ? <ActionButton label="Final Review" onClick={() => check(employee, 'payroll-final-review', 'Payroll Final Review')} disabled={!employee.payrollCheckedAt} /> : <span className="text-xs text-slate-500">Upper-level manager</span>}</Cell><Cell><ActionButton label="Manage" onClick={() => openEdit(employee)} /></Cell></tr>)}{!payroll.length && <EmptyRow columns={12} />}</tbody></table>
    </StatusSection>
