...
import { ArrowLeft, ClipboardEdit, Download, ExternalLink, HardHat, KeyRound, LogOut, Mail, Plus, RefreshCw, Search, ShieldCheck, XCircle } from 'lucide-react'
import { api } from '../shared/api'
import AccidentNearMiss from './AccidentNearMiss'
...
type CostEntry = { invoiceDate: string; description: string; paidBy: 'Workers Compensation' | 'Royal'; royalCostType: '' | 'Lost Time' | 'Medical Bill' | 'Other'; royalCostOtherItem: string; amount: number | string; invoiceLink: string }
...
type InjuryCase = {
  ...
  workersCompContactName: string
  workersCompContactPhone: string
  workersCompContactEmail: string
...
const emptyForm: CaseForm = {
  ...
  employeeInjuryFolderLink: '', injuryReportReceived: 'No', injuryReportLink: '', workersCompClaimed: 'No', workersCompCaseNumber: '', workersCompContactName: '', workersCompContactPhone: '', workersCompContactEmail: '',
...
function CaseEditor({ employees, existing, onCancel, onSaved }: { employees: Employee[]; existing: InjuryCase | null; onCancel: () => void; onSaved: () => void }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <form className="space-y-6 p-6" onSubmit={save}>
      ...
      <Field label={`Employee Injury Folder Link${existing ? '' : ' *'}`}><input className="control" onChange={event => set('employeeInjuryFolderLink', event.target.value)} placeholder="Secure SharePoint folder link" required={!existing} type="url" value={form.employeeInjuryFolderLink} /></Field>
      ...
      {form.workersCompClaimed === 'Yes' ? <section className="rounded-xl border border-indigo-200 bg-indigo-50/50 p-4"><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4"><Field label="Workers’ Compensation Case Number *"><input className="control" onChange={event => set('workersCompCaseNumber', event.target.value)} required value={form.workersCompCaseNumber} /></Field><Field label="WC Contact Name (Optional)"><input className="control" onChange={event => set('workersCompContactName', event.target.value)} value={form.workersCompContactName} /></Field><Field label="WC Contact Phone (Optional)"><input className="control" onChange={event => set('workersCompContactPhone', event.target.value)} type="tel" value={form.workersCompContactPhone} /></Field><Field label="WC Contact Email (Optional)"><input className="control" onChange={event => set('workersCompContactEmail', event.target.value)} type="email" value={form.workersCompContactEmail} /></Field></div></section> : null}
      ...
      {existing ? <section className="rounded-xl border border-amber-200 bg-amber-50/50 p-5"><div><h3 className="text-lg font-bold">Safety Violation &amp; Investigation</h3><p className="text-sm text-slate-600">Document the investigation findings and any corrective action required.</p></div><div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      {existing ? <section className="rounded-xl border border-emerald-200 bg-emerald-50/40 p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="text-lg font-bold">Case Costs</h3><p className="text-sm text-slate-600">Record invoices paid by Workers’ Compensation or Royal. Royal costs must be classified as Lost Time, Medical Bill, or Other.</p></div><button className="rounded-lg bg-emerald-700 px-3 py-2 text-sm font-bold text-white" onClick={() => setForm(current => ({ ...current, costs: [...current.costs, { invoiceDate: '', description: '', paidBy: 'Royal', royalCostType: '', royalCostOtherItem: '', amount: '', invoiceLink: '' }] }))} type="button">Add Cost</button></div><div className="mt-4 space-y-4">{form.costs.map((item, index) => <div className="grid gap-3 rounded-xl border border-emerald-100 bg-white p-4 md:grid-cols-2 lg:grid-cols-4" key={index}><Field label="Invoice Date (Optional)"><input className="control" onChange={event => updateCost(index, 'invoiceDate', event.target.value)} type="date" value={item.invoiceDate} /></Field><Field label="Paid By *"><select className="control" onChange={event => { updateCost(index, 'paidBy', event.target.value); if (event.target.value !== 'Royal') { updateCost(index, 'royalCostType', ''); updateCost(index, 'royalCostOtherItem', '') } }} value={item.paidBy}><option>Royal</option><option>Workers Compensation</option></select></Field>{item.paidBy === 'Royal' ? <Field label="Royal Cost Type *"><select className="control" onChange={event => { updateCost(index, 'royalCostType', event.target.value); if (event.target.value !== 'Other') updateCost(index, 'royalCostOtherItem', '') }} required value={item.royalCostType || ''}><option value="">Select cost type</option><option>Lost Time</option><option>Medical Bill</option><option>Other</option></select></Field> : null}{item.paidBy === 'Royal' && item.royalCostType === 'Other' ? <Field label="Other Cost Item *"><input className="control" onChange={event => updateCost(index, 'royalCostOtherItem', event.target.value)} placeholder="Describe the cost item" required value={item.royalCostOtherItem || ''} /></Field> : null}<Field label="Amount *"><input className="control" min="0" onChange={event => updateCost(index, 'amount', event.target.value)} required step="0.01" type="number" value={item.amount} /></Field><Field label="Description *"><input className="control" onChange={event => updateCost(index, 'description', event.target.value)} required value={item.description} /></Field><div className="lg:col-span-3"><Field label="Invoice Link"><input className="control" onChange={event => updateCost(index, 'invoiceLink', event.target.value)} placeholder="Secure SharePoint link" type="url" value={item.invoiceLink} /></Field></div><button className="self-end justify-self-start text-sm font-bold text-red-700" onClick={() => setForm(current => ({ ...current, costs: current.costs.filter((_, itemIndex) => itemIndex !== index) }))} type="button">Remove Cost</button></div>)}</div></section> : null}
...
function CaseDetails({ caseId, onLogout }: { caseId: string; onLogout: () => void }) {
  ...
  const royalLostTimeTotal = costs.filter(item => item.paidBy === 'Royal' && item.royalCostType === 'Lost Time').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const royalMedicalBillTotal = costs.filter(item => item.paidBy === 'Royal' && item.royalCostType === 'Medical Bill').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  const royalOtherTotal = costs.filter(item => item.paidBy === 'Royal' && item.royalCostType === 'Other').reduce((sum, item) => sum + Number(item.amount || 0), 0)
  ...
  return <div className="space-y-6 pb-10"><section className="rounded-2xl bg-slate-950 px-6 py-6 text-white shadow-xl"><div className="flex flex-wrap items-center justify-between gap-3"><Link className="inline-flex items-center gap-2 text-sm font-semibold text-slate-300 hover:text-white" to="/work-related-injury"><ArrowLeft className="h-4 w-4" />Back to Injury Cases</Link><button className="inline-flex items-center gap-2 text-sm font-semibold text-slate-300" onClick={onLogout}><LogOut className="h-4 w-4" />Sign out</button></div><div className="mt-6 flex flex-wrap items-center gap-3"><h1 className="text-3xl font-semibold">Work Injury Case Details</h1><span className={`rounded-full px-3 py-1 text-xs font-bold ${record.closedAt ? 'bg-slate-700 text-slate-200' : 'bg-emerald-500 text-white'}`}>{status}</span>{record.oshaRecordable === 'Yes' ? <span className="rounded-full bg-red-500 px-3 py-1 text-xs font-bold text-white">OSHA Recordable</span> : null}</div><p className="mt-2 text-slate-300">Complete read-only case record for {record.employeeName}.</p></section>
    ...
    <section className="rounded-2xl border border-orange-200 bg-white p-6 shadow-sm"><h2 className="mb-4 text-xl font-bold">2. Employee Injury Information</h2><DetailGrid items={injuryDetails} /><div className="mt-6 space-y-4"><DetailBlock label="Injury Description" value={record.injuryDescription} /></div><div className="mt-6 flex flex-wrap gap-3">{record.employeeInjuryFolderLink ? <a className="inline-flex items-center gap-2 rounded-lg bg-orange-700 px-4 py-2.5 text-sm font-bold text-white" href={record.employeeInjuryFolderLink} rel="noreferrer" target="_blank"><ExternalLink className="h-4 w-4" />Employee Injury Folder</a> : null}{record.injuryReportLink ? <a className="inline-flex items-center gap-2 rounded-lg bg-blue-700 px-4 py-2.5 text-sm font-bold text-white" href={record.injuryReportLink} rel="noreferrer" target="_blank"><ExternalLink className="h-4 w-4" />Injury Report</a> : null}</div></section>
    {record.workersCompClaimed === 'Yes' ? <section className="rounded-2xl border border-indigo-200 bg-white p-6 shadow-sm"><h2 className="mb-4 text-xl font-bold">Workers’ Compensation Contact</h2><DetailGrid items={[["Case Number", record.workersCompCaseNumber || '—'], ["Contact Name", record.workersCompContactName || 'Not provided'], ["Contact Phone", record.workersCompContactPhone || 'Not provided'], ["Contact Email", record.workersCompContactEmail || 'Not provided']]} /></section> : null}
    ...
    <section className="rounded-2xl border border-emerald-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">5. Case Costs</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><CostTotal label="Paid by Workers’ Compensation" value={wcTotal} /><CostTotal label="Royal — Lost Time" value={royalLostTimeTotal} /><CostTotal label="Royal — Medical Bill" value={royalMedicalBillTotal} /><CostTotal label="Royal — Other" value={royalOtherTotal} /><CostTotal label="Paid by Royal" value={royalTotal} /><CostTotal label="Total Case Cost" value={wcTotal + royalTotal} primary /></div>{costs.length ? <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[960px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Invoice Date</th><th className="px-4 py-3">Description</th><th className="px-4 py-3">Paid By</th><th className="px-4 py-3">Royal Cost Type</th><th className="px-4 py-3">Other Item</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Document</th></tr></thead><tbody>{costs.map((item, index) => <tr className="border-t border-slate-100" key={`${item.invoiceDate}-${index}`}><td className="px-4 py-3">{displayDate(item.invoiceDate)}</td><td className="px-4 py-3 font-semibold">{item.description}</td><td className="px-4 py-3">{item.paidBy}</td><td className="px-4 py-3">{item.paidBy === 'Royal' ? item.royalCostType || 'Uncategorized' : '—'}</td><td className="px-4 py-3">{item.royalCostType === 'Other' ? item.royalCostOtherItem || '—' : '—'}</td><td className="px-4 py-3 font-bold">{Number(item.amount).toLocaleString('en-US', { style: 'currency', currency: 'USD' })}</td><td className="px-4 py-3">{item.invoiceLink ? <a className="font-bold text-blue-700" href={item.invoiceLink} rel="noreferrer" target="_blank">Open Invoice</a> : '—'}</td></tr>)}</tbody></table></div> : <p className="mt-5 rounded-xl bg-slate-50 p-5 text-sm text-slate-500">No costs have been recorded.</p>}</section>
  </div>
}
...
function InjuryWorkspace({ currentEmail, onLogout }: { currentEmail: string; onLogout: () => void }) {
  ...
  const [costReportFrom, setCostReportFrom] = useState('')
  const [costReportTo, setCostReportTo] = useState('')
  ...
  async function declineClose() {
    if (!selected || !window.confirm(`Decline the closure request for ${selected.employeeName}? The case will become editable again and administrators may submit a new closure request after making changes.`)) return
    try { await api.post(`/work-injury/cases/${selected.id}/close-decline`); setSelectedId(''); await load() }
    catch (requestError: any) { setError(requestError.response?.data?.error || 'The closure request could not be declined.') }
  }
  ...
  return <div className="space-y-6 pb-10">
    ...
    <div className="flex rounded-xl border border-slate-200 bg-white p-1 shadow-sm"><button className={`flex-1 rounded-lg px-4 py-3 text-sm font-bold ${tab === 'injury' ? 'bg-orange-700 text-white shadow' : 'text-slate-600 hover:bg-slate-50'}`} onClick={() => setTab('injury')}>Work Injury</button><button className={`flex-1 rounded-lg px-4 py-3 text-sm font-bold ${tab === 'accident' ? 'bg-blue-700 text-white shadow' : 'text-slate-600 hover:bg-slate-50'}`} onClick={() => setTab('accident')}>Accident &amp; Near Miss</button></div>
    {tab === 'accident' ? <AccidentNearMiss /> : <>
      <section className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5 shadow-sm"><div className="flex flex-wrap items-end gap-3"><div className="mr-auto"><h2 className="text-lg font-bold text-emerald-950">Work Injury Reports</h2><p className="mt-1 text-sm text-emerald-800">Download complete case history or summarize costs by injury date.</p></div><a className="inline-flex items-center gap-2 rounded-lg bg-emerald-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-800" href="/api/work-injury/reports/case-details.xlsx"><Download className="h-4 w-4" />All Case Details</a><label><span className="block text-xs font-bold uppercase text-emerald-800">Injury Date From</span><input className="control mt-1" onChange={event => setCostReportFrom(event.target.value)} type="date" value={costReportFrom} /></label><label><span className="block text-xs font-bold uppercase text-emerald-800">Injury Date To</span><input className="control mt-1" onChange={event => setCostReportTo(event.target.value)} type="date" value={costReportTo} /></label><a aria-disabled={!costReportFrom || !costReportTo || costReportFrom > costReportTo} className={`inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-bold text-white ${costReportFrom && costReportTo && costReportFrom <= costReportTo ? 'bg-slate-800 hover:bg-slate-900' : 'pointer-events-none bg-slate-400'}`} href={costReportFrom && costReportTo ? `/api/work-injury/reports/case-costs.xlsx?from=${costReportFrom}&to=${costReportTo}` : undefined}><Download className="h-4 w-4" />Case Cost Report</a></div></section>
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><button className="inline-flex items-center justify-center gap-2 rounded-xl bg-orange-700 px-5 py-4 font-bold text-white shadow-sm hover:bg-orange-800" onClick={() => setView('new')}><Plus className="h-5 w-5" />New Case</button><button className="inline-flex items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-5 py-4 font-bold text-blue-800 disabled:cursor-not-allowed disabled:opacity-40" disabled={!selected || Boolean(selected.closeRequestedAt)} onClick={() => setView('edit')}><ClipboardEdit className="h-5 w-5" />Edit Case</button>{selected?.closeRequestedAt ? currentEmail.toLowerCase() === 'myu@royaltrailersales.com' ? <><button className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-300 bg-red-700 px-5 py-4 font-bold text-white" onClick={finalApproveClose}><ShieldCheck className="h-5 w-5" />Final Approve &amp; Close</button><button className="inline-flex items-center justify-center gap-2 rounded-xl border border-amber-300 bg-amber-50 px-5 py-4 font-bold text-amber-800" onClick={declineClose}><XCircle className="h-5 w-5" />Decline Closure</button></> : <button className="inline-flex items-center justify-center gap-2 rounded-xl border border-purple-200 bg-purple-50 px-5 py-4 font-bold text-purple-800" disabled><ShieldCheck className="h-5 w-5" />Waiting for Upper-Level Manager Approval</button> : <button className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-5 py-4 font-bold text-red-700 disabled:cursor-not-allowed disabled:opacity-40" disabled={!selected} onClick={closeCase}><XCircle className="h-5 w-5" />Request Case Closure</button>}</section>
